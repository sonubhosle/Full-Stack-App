...
const createUser = async (userData) => {
    try {
        let { name, surname, email, password, confirmPassword, mobile, photo, role } = userData;
        ...
        if (!passwordRegex.test(password)) {
            throw new Error("Password must be at least 8 characters, include uppercase, number and symbol");
        }
        ...
        const user = await User.create({ surname, name, password, mobile, photo, email, role });
...
module.exports = { createUser, updateUserProfile, findUserById, findUserByEmail, getUserProfileByToken, getAllUsers, setResetPasswordToken, resetPassword };
